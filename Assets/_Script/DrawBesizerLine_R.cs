...
public class DrawBesizerLine_R : MonoBehaviour
{
    Transform[] jjd;
    ...
    void Start()
    {
        jjd = Create.GetComponentsInChildren<Transform>();
    ...
    void Update()
    {
        ...
        foreach (Transform obj1 in jjd)
        {
            if (obj1.transform.name == "A_T test(Clone)" || obj1.transform.name == "T_A test (Clone)" || obj1.transform.name == "C_G test(Clone)" || obj1.transform.name == "G_C test(Clone)")
            {
                obj1.transform.gameObject.SetActive(false);
            }
        }
    }
    ...
    {
        //获取指定的点的信息
        Vector3[] pointPos = new Vector3[basePoint_R.Count];
        for (int i = 0; i < pointPos.Length; i++)
        {
            pointPos[i] = basePoint_R[i].transform.position;
        }
